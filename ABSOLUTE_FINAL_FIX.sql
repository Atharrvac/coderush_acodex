-- ============================================
-- ABSOLUTE FINAL FIX - Works with existing tables
...
-- Step 5: Fix achievements table - add missing columns
DO $$ 
BEGIN
  -- Check what columns exist and add missing ones
  ALTER TABLE achievements ADD COLUMN IF NOT EXISTS points INTEGER DEFAULT 0;
  ALTER TABLE achievements ADD COLUMN IF NOT EXISTS badge_color VARCHAR(20) DEFAULT 'blue';
...
-- Step 6: Clear and re-insert achievements (safe way)
...
-- Step 8: Create activity feed function with SECURITY DEFINER
...
-- Step 9: Create vote counting function
...
-- Step 12: Drop all existing policies
...
-- Step 15: Create chat system functions
