...
-- This will fix ALL errors and make everything work
-- ============================================
...
-- Step 2: Create tables if they don't exist (safe approach)
...
-- Step 3: Add missing columns to existing tables (safe - won't fail if exists)
DO $$ 
BEGIN
  ...
  -- Add points column to achievements if it doesn't exist
  BEGIN
  ...
  -- Drop points_required if it exists (old column name)
  BEGIN
    ALTER TABLE achievements DROP COLUMN IF EXISTS points_required;
  EXCEPTION
...
-- Step 5: Insert default achievements (safe - won't duplicate)
...
-- Step 6: Create activity feed function with SECURITY DEFINER
...
-- Step 7: Create vote counting function
...
DROP POLICY IF EXISTS "authenticated_users_all" ON problem_votes;
DROP POLICY IF EXISTS "authenticated_users_all" ON achievements;
DROP POLICY IF EXISTS "authenticated_users_all" ON user_achievements;
DROP POLICY IF EXISTS "authenticated_users_all" ON activity_feed;
DROP POLICY IF EXISTS "authenticated_users_all" ON user_points;
DROP POLICY IF EXISTS "authenticated_users_all" ON help_sessions;
DROP POLICY IF EXISTS "authenticated_users_all" ON chat_messages;
DROP POLICY IF EXISTS "authenticated_users_all" ON session_updates;

-- Step 11: Create permissive policies for authenticated users
...
-- Step 13: Create chat system functions
